...
**State (2026-09-25):** the plan P0–P6 in `backlog.md` is complete. The stand builds from scratch, `make verify` gives 38 PASS / 0 FAIL, and the failure tests `hack/tests/h41…h49`, `h62` were run on this stack. The parent's full backlog is `git show 7279079:backlog.md`.
...
| `hack/tests/` | Failure-test scripts and analyzers: `h41-push-pull.sh`, `h42-kill-during-push.sh`, `h43-rolling-update.sh`, `h44-node-loss.sh`, `h45-app-rollout.sh`, `h46-proxy-cache.sh`, `h47-role-failure.sh`, `h48-node-replace.sh`, `h49-trivy-node-loss.sh`, `h62-sync-mode.sh` (+ `h4x_analyze.py`) |
