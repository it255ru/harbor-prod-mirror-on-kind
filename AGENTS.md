...
| `hack/tests/` | Failure-test scripts and analyzers: `h41-push-pull.sh`, `h42-kill-during-push.sh`, `h43-rolling-update.sh`, `h44-node-loss.sh`, `h45-app-rollout.sh`, `h46-proxy-cache.sh`, `h47-role-failure.sh`, `h48-node-replace.sh`, `h49-trivy-node-loss.sh`, `h50-stateful-node-replace.sh` (+ `lib-node.sh`), `h62-sync-mode.sh` (+ `h4x_analyze.py`) |
