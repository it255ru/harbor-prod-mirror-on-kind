...
| `h50-stateful-node-replace.sh <pg\|redis\|consul\|s3\|lb>` (uses `lib-node.sh`) | permanent loss and replacement of the node of a role holder with empty volumes; checks the data and the self-healing, applies the documented repair (PVC reset; s3: data loss and Harbor clean-up) | 6-10 min each |
...
| `consul` / `redis` / `pg` node lost for good and replaced (`h50`) | data intact (consul 10 of 10 keys, redis 50 of 50, pg 200 of 200 rows). The role heals by itself: consul in ~2 min, pg in ~2 min and redis in ~1.8 min after the loss (an init container `fix-perms` hands the new node's root-owned volume directory to the database user; before it, pg and redis got `Permission denied` and needed the PVC and the pod deleted) | intact |
