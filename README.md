...
Fork of [harbor-active-active-on-kind](https://github.com/it255ru/harbor-active-active-on-kind) (@ `7279079`, 2026-09-25, full history kept; published as https://github.com/it255ru/harbor-prod-mirror-on-kind) aimed at one specific production stand rather than the parent's general HA scheme: Harbor **2.14.3** (chart `1.18.3`, not 2.15.2/1.19.2), **HAProxy + Keepalived** as the Infra LB (not MetalLB/ingress-nginx), external PostgreSQL downgraded to **15.19** (not 18.6). Redis Sentinel, Patroni + Consul and Garage are unchanged from the parent — decisions D11-D14 in [backlog.md](backlog.md).
