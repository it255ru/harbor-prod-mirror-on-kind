#!/usr/bin/env bash
# H4.10 (backlog): PERMANENT loss and replacement of a node that holds state (pg | redis | consul | s3).
# The volumes are local-path PVs bound to the node: the replacement node has EMPTY volumes.
#
# usage: hack/tests/h50-stateful-node-replace.sh <pg|redis|consul|s3> [node]
...
set -uo pipefail
ROLE=${1:?usage: h50-stateful-node-replace.sh <pg|redis|consul|s3> [node]}; NODE=${2:-}
...
NS=harbor-deps; PG_IMAGE=${PG_IMAGE:-harbor-ha/patroni:4.1.5-pg15.19}
...
POD=$(kubectl -n $NS get pods -o custom-columns=N:.metadata.name,NODE:.spec.nodeName --no-headers | awk -v n="$NODE" '$2==n && $1 ~ /^(pg|redis|consul|garage)-/{print $1; exit}')
...
sleep 30
echo "   state with the node lost:"; case "$ROLE" in pg) pg_json | python3 -c "import sys,json; [print('     ',m['Member'],m['Role'],m['State']) for m in json.load(sys.stdin)]" ;; redis) echo "     master now: $(redis_master)";; consul) echo "     leader now: $(consul_leader)";; s3) echo "     Garage is gone: registry data path down";; esac
